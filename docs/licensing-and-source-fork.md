...
MinaFox is currently a wrapper/profile distribution around the system Firefox package. This document keeps the current Arch package phase separate from any future Firefox ESR source-fork phase.
...
3. depends on the distro/system Firefox package (`firefox`) instead of replacing it;
...
MinaFox-owned code is licensed under MPL-2.0 in [`../LICENSE`](../LICENSE). Third-party component notes live in [`../THIRD_PARTY_LICENSES.md`](../THIRD_PARTY_LICENSES.md).
...
- avoid `provides=('firefox')` or `conflicts=('firefox')`;
